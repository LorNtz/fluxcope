...
- 设计审查无阻塞问题；性能审查指出旧公开报告过期时会先重跑旧 tag workflow。
- 已调整优先选择拥有准确 formula 的 master verifier，避免已知失败路径和额外安装编译；新增过期证据回归覆盖。
- 发布指南明确安装工具可在 master 修复，应用或产物变化仍需新版本。

### 首版安装验证：稳定自动按键时序
Date: 2026/09/15 12:26

- ARM Linux 已成功安装并通过 binary 摘要检查，但 PTY 测试的 Esc 与 q 合并导致停在设置页。
- 原 key() 使用单次 select timeout，在屏幕持续输出时立即返回；改为在完整 200ms 间隔内持续读取，避免独立按键被解析为 Alt 组合。
- 应用源码与发布产物未修改；增加真实 PTY 输入回归并验证正式 ARM macOS binary。
