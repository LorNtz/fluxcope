...
if [[ "$CLIENT_PHASE" == build ]]; then
  "$PYTHON" "$CONTROLLER/scripts/preview_client_build.py" python --target "$TARGET" --directory /tmp/preview-python
  /tmp/preview-python/python/bin/python3 -m venv /tmp/preview-client-venv
